...
# Anime Frame Guesser - Project Guidelines
...
## Architecture & Rules
1. **Modular Components:** Single-responsibility components in `src/components/`.
2. **Flexible Data Scheme:** Every frame item in JSON must include `id`, `animeId` (e.g. `jojo`, `naruto`), `image`, `part` (or season), `episode`, and `title`.
3. **Asset Paths:** Images are stored in `public/obrazky/` and frame metadata in `public/data/frames.json`.

subtle borders (`border-white/10`).
5. **Gameplay Mechanics:** Framed/Wordle style guess history with visual indicators (green for correct match, ⬆️/⬇️ arrows for episode deviation).